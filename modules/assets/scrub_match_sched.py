...
def prompt_threshold(default=94):
    """Prompt user for similarity threshold and return as float (e.g., 0.94)."""
    try:
        default_int = int(float(default) * 100) if isinstance(default, str) else int(default)
    except Exception:
        default_int = 94  # Safe fallback if conversion fails
    ...
    return val / 100 if ok else default_int / 100